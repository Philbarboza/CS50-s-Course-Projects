// Week 3 Challenge - Runoff - Introduction to Computer Science Harvard

// Author: Philipp Pfister (Responsible from line 143 till the end)

// Implement a program that runs a a ranked-choice voting system. 
...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            int pc = preferences[i][j];
            if (candidates[pc].eliminated == false)
            {
                candidates[pc].votes++;
                break;
            }
...
int find_min(void)
{
    int min[1] = {MAX_VOTERS}; // create an array called min, where the temporary minimum value is saved
    for (int c = 0; c < candidate_count; c++)
    {
        if (candidates[c].votes < min[0] && candidates[c].eliminated == false)
        {
